      ...
      if(r.success) {
          ...
          $.each(r.champions, function (index, value) {
              ...
              if(value.games === undefined){
                ...
                $('#championsTable').append('<tr><td>' + counter + '</td> <td class="mdl-data-table__cell--non-numeric"> <img src="http://ddragon.leagueoflegends.com/cdn/6.8.1/img/champion/' + value.championKey + '.png" class="champions-table-img">' + value.championName + '</td> <td>' + value.championLevel + '</td> <td>' + value.championPoints + '</td> <td>' + value.games + '</td> <td>' + winrate + '%</td><td>' + value.kda + '</td></tr>');
              }
              counter++;
          });
      }
